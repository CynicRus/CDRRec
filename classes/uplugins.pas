...
uses
  Classes, SysUtils {$IFDEF WINDOWS}, JwaWindows {$ENDIF}, dynlibs, gvector,
  lpcompiler, lptypes, lpffiwrappers, ffi, syncobjs;
type
  TMPlugin = class;
  TMPluginDeclaration = class;
  TMPluginDeclarations = class(specialize TVector<TMPluginDeclaration>);
  ...
  TMPluginsList = specialize TVector<TMPlugin>;
  TMPlugins = class
  protected
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        FDeclarations.PushBack(TMPluginType.Create(Name, Str));
      end;
    finally
      StrDispose(Name);
      StrDispose(Str);
  ...
  begin
    ...
    try
      ...
      begin
        ...
        FDeclarations.PushBack(TMPluginMethod.Create(Address, Header));
      end;
    finally
      StrDispose(Header);
    end;
  ...
  begin
    ...
    try
      ...
      FDeclarations.PushBack(TMPluginCode.Create(Str, ExtractFileNameOnly(FFilePath)));
    finally
      StrDispose(Str);
    end;
  end;
...
begin
  ...
  try
    SetLength(Files, FPlugins.Size);
    SetLength(RefCounts, FPlugins.Size);
...
begin
  ...
  FPaths.Add(ExtractFilePath(ParamStr(0))+'plugins\');
end;
